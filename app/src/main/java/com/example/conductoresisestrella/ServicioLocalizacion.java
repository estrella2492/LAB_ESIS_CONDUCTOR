package com.example.conductoresisestrella;
...
public class ServicioLocalizacion extends Service {
    ...
    @Override public void onCreate() {
        ...
        locationRequest.setPriority(LocationRequest.PRIORITY_HIGH_ACCURACY); //para optimizar el consumo de bateria
        locationRequest.setInterval(10 * 1000); // 10 seconds --> intervalo normal
        locationRequest.setFastestInterval(5 * 1000); // 5 seconds --> intevalo rapido
        locationCallback = new LocationCallback() {
            @Override
            public void onLocationResult(LocationResult locationResult) { //metodo para ubicar la localizacion cuando cambia
        ...
        mFusedLocationClient.requestLocationUpdates(locationRequest, locationCallback, null); //cada vez que se actuliza la posicion se actualiza y comunica
    }
    @Override
    public int onStartCommand(Intent intenc, int flags, int idArranque) {
        Toast.makeText(this,"Servicio localizaciones arrancado ",
                Toast.LENGTH_SHORT).show();
        NotificationCompat.Builder notific = new NotificationCompat.Builder(this) //es obligatorio para Android 8 y 9
